...
class ComplianceService:
    ...
    @staticmethod
    def export_subject_data(tenant_id: int, subject_identifier: str, requester_user_id: Optional[int] = None) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def erase_subject_data(tenant_id: int, subject_identifier: str, requester_user_id: Optional[int] = None) -> Dict[str, Any]:
        """
        Anonimiza irreversivelmente os dados do titular atendendo ao Direito ao Esquecimento (LGPD Art. 18, VI).
        """
        logger.info(f"Executing LGPD erasure for subject '{subject_identifier}' on tenant {tenant_id}")
        ...
        for item in analyses:
            ...
            if item.email_content:
                item.email_content = item.email_content.replace(subject_identifier, "[LGPD_ANONYMIZED]")
            if item.suggested_response:
                item.suggested_response = item.suggested_response.replace(subject_identifier, "[LGPD_ANONYMIZED]")
            ...
            if item.extracted_entities and isinstance(item.extracted_entities, dict):
                ...
                for k, v in item.extracted_entities.items():
                    if isinstance(v, str) and subject_identifier in v:
                        ...
                        cleaned_entities[k] = ["[LGPD_ANONYMIZED]" if (isinstance(elem, str) and subject_identifier in elem) else elem for elem in v]
                    else:
    ...
    @staticmethod
    def run_retention_cleanup(tenant_id: int, retention_days: Optional[int] = None, requester_user_id: Optional[int] = None) -> Dict[str, Any]:
        """
