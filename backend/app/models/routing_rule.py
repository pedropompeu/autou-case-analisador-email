...
class RoutingRule(BaseModel):
    ...
    # Condição
    condition_field = db.Column(
        db.String(50), nullable=False
    )  # sentiment, urgency, category, fraud_risk_score
    condition_operator = db.Column(
        db.String(30), nullable=False, default="equals"
    )  # equals, contains, gte, in
    condition_value = db.Column(db.String(255), nullable=False)

    # Ação
    action_type = db.Column(
        db.String(50), nullable=False
    )  # set_quarantine, set_urgency, alert_webhook, assign_role
